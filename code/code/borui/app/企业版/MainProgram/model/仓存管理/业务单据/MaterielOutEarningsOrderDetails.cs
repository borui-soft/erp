...
namespace MainProgram.model
{
    ...
    public class MaterielOutEarningsOrderDetails : ITableModel
    {
        private SortedDictionary<int, MaterielOutEarningsOrderDetailsTable> m_tableDataList = new SortedDictionary<int, MaterielOutEarningsOrderDetailsTable>();

        static private MaterielOutEarningsOrderDetails m_instance = null;

        private MaterielOutEarningsOrderDetails()
        {
            load();
        }

        static public MaterielOutEarningsOrderDetails getInctance()
        {
            if (m_instance == null)
            {
                m_instance = new MaterielOutEarningsOrderDetails();
            }
        ...
        public void insert(ArrayList orderDetailsRecords)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        MaterielOutEarningsOrderDetailsTable record = (MaterielOutEarningsOrderDetailsTable)orderDetailsRecords[i];
                        ...
                        string insert = "INSERT INTO [dbo].[WAREHOUSE_MANAGEMENT_OUT_EARNINGS_DETAILS]([ROW_NUMBER],[MATERIEL_ID],[BILL_NUMBER]";
        ...
        {
            string delete = "DELETE FROM WAREHOUSE_MANAGEMENT_OUT_EARNINGS_DETAILS WHERE BILL_NUMBER = '" + billNumber + "'";
        ...
        public SortedDictionary<int, MaterielOutEarningsOrderDetailsTable> getAllMaterielOutEarningsOrderInfo()
        {
        ...
        public SortedDictionary<int, MaterielOutEarningsOrderDetailsTable> getMaterielOutEarningsInfoFromBillNumber(string billNumber)
        {
            SortedDictionary<int, MaterielOutEarningsOrderDetailsTable> list = new SortedDictionary<int, MaterielOutEarningsOrderDetailsTable>();

            foreach (KeyValuePair<int, MaterielOutEarningsOrderDetailsTable> index in m_tableDataList)
            {
        ...
        public SortedDictionary<int, MaterielOutEarningsOrderDetailsTable> getMaterielOutEarningsOrderCountInfo(string startDate, string endDate, bool isAllBill = false)
        {
            SortedDictionary<int, MaterielOutEarningsOrderDetailsTable> dataList = new SortedDictionary<int, MaterielOutEarningsOrderDetailsTable>();
            string querySQL = "SELECT A1.[PKEY],A1.[ROW_NUMBER],A1.[MATERIEL_ID], A1.[BILL_NUMBER],A1.[PRICE],A1.[VALUE],A1.[NOTE] ";
            querySQL += "FROM [WAREHOUSE_MANAGEMENT_OUT_EARNINGS_DETAILS] A1, WAREHOUSE_MANAGEMENT_OUT_EARNINGS A2";
            querySQL += " WHERE A1.BILL_NUMBER = A2.BILL_NUMBER";
    ...
    public class MaterielOutEarningsOrderDetailsTable
    {
